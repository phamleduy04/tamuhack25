...
route.get('/', async (req, res) => {
    ...
    const response = await flightawareClient.get(`/flights/${icao24}?start=${now.subtract(1, 'day').format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')}&end=${now.add(12, 'hours').format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')}`);
